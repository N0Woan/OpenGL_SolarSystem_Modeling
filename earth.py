...
class Earth(object):
    def __init__(self, radius, sides, orbit_radius, orbit_sides, vert_shader, frag_shader):   
    ...
    def draw(self, projection, view, model):
        GL.glUseProgram(self.shader.render_idx)
        self.frame %= len(self.orbit)
        trans = translate(vec((self.orbit[self.frame][0], self.orbit[self.frame][1], self.orbit[self.frame][2])))
        self.rot = rotate(vec((0,-0.3971479,0.91775465)), angle=360/96) @ self.rot
        modelview = view @ trans @ self.rot
        self.frame += 1
